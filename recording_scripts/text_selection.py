...
# -*- coding: utf-8 -*-
'''
Selecciona de un texto, las frases más adecuadas para ser, posteriormente, utilizadas
como fuentes para la generación de audios emparejados consu texto correspondiente.
'''
import codecs
import re
...
text_original = 'textos/frases.txt'
arxiu_scores = 'textos/frases_score.txt'
arxiu_frases = 'textos/frases_trans.txt'
...
def broadSelection(corpora):
    ...
    for sentence in sentences:
        ...
        if not re.findall(r'[0-9]', sentence) and len(sentence) < 101:
...
def scoreSelection(transcripted_sentences):
    ...
    for transcription in transcripted_sentences:
       ...
       f.write(f"{sc[0]}"); f.write(f"{sc[1]}"); f.write(f"\n")
       scores.append(sc)
